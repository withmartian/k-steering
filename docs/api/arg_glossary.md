...
This section documents the configuration arguments used throughout the `k-steering` package. It is intended to serve as a single reference point for understanding _what each parameter means_, _where it is used_, and _how it affects steering behavior_.
